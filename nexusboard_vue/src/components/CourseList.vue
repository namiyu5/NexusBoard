  ...
  <section class="w-full max-w-6xl mx-auto px-6 py-12">
    ...
    <div class="grid md:grid-cols-3 gap-8" v-if="!loading && !error">
      <div v-for="course in courses" :key="course.id" class="card">
        ...
        <p class="text-sm muted mb-4">{{ course.excerpt || course.description || '' }}</p>
        <div class="flex justify-between items-center">
          <span class="text-xs muted">Lessons: {{ course.lessons ? course.lessons.length : '—' }}</span>
          <router-link :to="`/courses/${course.id}`" class="btn btn-primary">View</router-link>
        </div>
      </div>
    </div>
  </section>
